...
ZipArchiveOutputStream.prototype._appendStream = function(ae, source, callback) {
  ...
  this._writeLocalFileHeader(ae);
  if (ae.getCompressedSize() > 0 && this.offset < this._range.start && this.offset + ae.getCompressedSize() <= this._range.end) {
    this.offset += ae.getCompressedSize();
    this._afterAppend(ae);
    callback(null, ae);
    return;
  }
...
ZipArchiveOutputStream.prototype._smartStream = function(ae, callback) {
  var compressed = (ae.getCompressedSize() > 0 && ae.getCrc() > 0);
  var deflate = ae.getMethod() === constants.METHOD_DEFLATED;
  var process = compressed ? new CRC32Stream() : deflate ? new DeflateCRC32Stream(this.options.zlib) : new CRC32Stream();
